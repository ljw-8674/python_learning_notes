...
`len()` 函数计算的是str的字符数，如果换成bytes，就计算字节数:
...
可以用索引来访问list中每一个位置的元素，索引是从0开始，其次为 1，2，3，... ，要确保索引不要越界。也可以从右往左计数，依次为 -1，-2，-3，... 。
...
match语句还可以匹配复杂的数据结构，例如list。

```python
args = ['gcc', 'input.c', 'output.c']
# args = ['clean']
# args = ['gcc']
...
在循环中，`break` 语句可以提前退出循环，适用于找到目标、用户输入退出等场景。`continue` 语句则可以跳过当前的这次循环，直接开始下一次循环，适用于跳过某些不符合条件的元素。
...
注意：返回 `None` 的时候Python的交互式环境不显示结果。
...
要保证hash的正确性，作为key的对象就不能变。在Python中，字符串、整数等都是不可变的，因此，可以放心地作为 key。而list是可变的，就不能作为key，如下所示：
...
- 位置参数。最常见的参数，必须按顺序传，且数量必须一致。
- 默认参数。定义时给默认值，调用时可传可不传。默认参数放在位置参数后面。
- 可变位置参数。用 `*` 接收任意个位置参数，传入函数时会打包成tuple。 
- 关键字参数。只能用 `key=value` 方式传，通常出现在可变位置参数之后。
- 可变关键字参数。用 `**` 接收任意个关键字参数，传入函数时会打包成dict。
...
- 如果函数需要处理的问题大小合适，则直接求解并返回结果；
- 否则将问题分解成若干个更小的子问题，并对子问题进行相同的处理，直到问题无法分解为止。
...
- 在处理复杂问题时提供一种简单且易懂的解决方案；
- 递归函数使代码具有可读性和可重用性，且可以使用递归函数解决使用其他方法难以处理的问题。

递归函数的缺点: 

- 递归函数可能会在运行时占用较多的系统资源，因为它们需要在堆栈上存储多个函数调用；
- 其次，递归函数可能导致代码变得不容易理解，因为它具有一定的复杂度。

著名的汉诺塔问题（Tower of Hanoi）求解如下：
...
def hanoi(num, source, auxiliary, target):
    ...
    if num == 1:
        ...
        # 第一步：将n-1个盘子从【源柱】借助【目标柱】移动到【辅助柱】
        hanoi(num-1, source, target, auxiliary)
        
        # 第二步：将最大的盘子从【源柱】移动到【目标柱】
        count += 1
        print(f'第{count}次移动圆盘，{source} --> {target}')
        
        # 第三步：将n-1个盘子【源柱】从借助【辅助柱】移动到【目标柱】
